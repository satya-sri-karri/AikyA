...
npm install
cp .env.example .env
# edit .env and fill in MONGODB_URI and XAI_API_KEY (get it at https://console.x.ai)
...
`aiService.js` sends that context to the xAI Grok API (`grok-4.6`) along with the
...
## 6. Admin panel (add if time allows)

Every collection already has full CRUD routes (`POST`, `PUT`, `DELETE` on
`/api/departments`, `/api/faculty`, `/api/poi`, `/api/buses`, `/api/events`).
A basic admin UI just needs simple forms hitting these endpoints — no new
